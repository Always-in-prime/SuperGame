...
//  по extended-биту (bit 24) в lParam.
// ---------------------------------------------------------------------
static WPARAM NormalizeExtendedVK(WPARAM vk, LPARAM lParam) noexcept {
    if (vk == VK_CONTROL)
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KILLFOCUS:
        ...
        // WM_PAINT больше не рисует игру Ч отрисовка идЄт из App_Frame.
        // ¬алидируем регион, чтобы Windows не слала WM_PAINT бесконечно.
    case WM_PAINT: {
        PAINTSTRUCT ps;
        BeginPaint(hwnd, &ps);
        EndPaint(hwnd, &ps);
